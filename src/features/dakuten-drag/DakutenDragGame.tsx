...
function loadShuffledQuestions(unitId?: string): DragQuestion[] {
  const lesson = unitId
    ? (LESSONS.find(
        (l) => l.unit_id === unitId && (l.dakuten_drag_items?.length ?? 0) > 0,
...
export default function DakutenDragGame() {
  ...
  const [questions, setQuestions] = useState<DragQuestion[]>(() =>
    loadShuffledQuestions(
      typeof advUnitId === 'string' ? advUnitId : undefined,
    ),
  );
          ...
          <button
            ...
            onClick={() =>
              adventure.submitResult(
                accuracy,
                calculateXpGain(score, questions.length),
              )
            }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center gap-6 p-4 pt-6 bg-gradient-to-b from-violet-100 to-pink-50">
      ...
      <div className="flex flex-col items-center gap-3">
        <div className="text-6xl font-bold text-violet-700">
          {current.item.meaning_zh}
        </div>
        <button
          type="button"
      ...
      <p className="text-2xl text-gray-600 text-center">
        {t('dakutenQuestion').split('{mark}')[0]}
        <span className="text-4xl font-bold text-red-500 mx-1">
          {markSymbol}
        </span>
        {t('dakutenQuestion').split('{mark}')[1]}
      </p>
      ...
      <div className="flex flex-col items-center gap-2 mt-2">
        <button
          type="button"
          aria-label={t('dakutenDragAria').replace('{mark}', markSymbol)}
          ...
          className={[
            ...
            dragPos
              ? 'invisible'
              : 'cursor-grab hover:scale-110 transition-transform active:scale-95',
        ...
        {!dragPos && (
          <p className="text-lg text-gray-400">{t('dakutenDragHint')}</p>
        )}
      </div>
