...
export default function KanaCatchGame() {
  ...
  const { ageMode, kanaDifficulty, addStars, startSession, endSession } =
    useAppStore();
  ...
  const kanaPool = useMemo(
    () => buildPool(ALL_KANA, ageMode, kanaDifficulty),
  ...
  const handleComplete = useCallback(
    async (correct: number, total: number) => {
      ...
      const result = await addXpToPet(
        xp,
        subMode !== 'word_to_image' ? correctIds.current : [],
      );
      setPet(result.pet);
      if (result.leveledUp) {
        playSfx('levelup');
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      const maxB =
        paramsOverride.maxBubbles ?? (effectiveParams.showRomaji ? 3 : 5);
      ...
      const engine = new KanaCatchEngine(canvas, effectiveParams, {
        ...
        onCorrect: (id) => {
          void handleCorrect(id);
        },
        onMiss: (id) => {
          void handleMiss(id);
        },
        onComplete: (c, t) => {
          void handleComplete(c, t);
        },
      });
      engine.start();
  ...
  const handleStart = useCallback(
    (mode: KanaCatchSubMode, uid?: string) => {
  ...
  useEffect(() => {
    ...
    if (Array.isArray(ov.unitPool)) {
      const poolLessons = LESSONS.filter((l) =>
        (ov.unitPool as string[]).includes(l.unit_id),
      );
    ...
    if (typeof ov.roundLength === 'number')
      override.roundLength = ov.roundLength;
    ...
    if (typeof ov.highlightCorrectAnswer === 'boolean')
      override.highlightCorrectAnswer = ov.highlightCorrectAnswer;
  ...
  if (gameState === 'results') {
    const accuracy =
      effectiveParams.roundLength > 0 ? score / effectiveParams.roundLength : 0;
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-8 p-6">
        ...
        <p className="text-3xl text-gray-600">
          {score} / {effectiveParams.roundLength} {t('correct')}
        </p>
        {adventure.isActive ? (
          <button
            type="button"
            onClick={() =>
              adventure.submitResult(
                accuracy,
                calculateXpGain(score, effectiveParams.roundLength),
              )
            }
          ...
          <>
            ...
            <button
              type="button"
              aria-label={t('kanaCatchChangeMode')}
  ...
  const progress = Math.max(
    0,
    ((questionNum - 1) / effectiveParams.roundLength) * 100,
  );
  ...
  return (
    <div className="min-h-screen flex flex-col items-center gap-3 p-4 pt-6 bg-gradient-to-b from-orange-50 to-white">
      ...
      <div
        className={`flex flex-col items-center gap-1 transition-all duration-300 ${bounce ? '-translate-y-4 scale-110' : ''}`}
      >
        {pet ? (
