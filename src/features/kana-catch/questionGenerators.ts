...
export function buildPool(
  ...
  if (
    ageMode === 'young' &&
    (kanaDifficulty === 1 || kanaDifficulty === 'all')
  ) {
...
export function buildParams(ageMode: AgeMode): {
  fallSpeed: number;
  showRomaji: boolean;
  roundLength: number;
} {
  // B4.1: slower defaults so young learners have time to read and tap
...
export function makeListenGenerator(
  ...
  return () => {
    ...
    const correct = srsWeightedPick(
      queue.slice(idx, idx + Math.min(10, queue.length)),
      (k) => k.id,
      pMap,
    );
    idx++;
    const count = Math.min(maxBubbles - 1, kanaPool.length - 1);
    const others = shuffled(kanaPool.filter((k) => k.id !== correct.id)).slice(
      0,
      count,
    );
...
export function makeMinimalPairGenerator(
  ...
  return () => {
    ...
    const distractors = shuffled(
      pairKana.filter((k) => k.id !== correct.id),
    ).slice(0, maxBubbles - 1);
    const items: BubbleItem[] = shuffled([correct, ...distractors]).map(
      (k) => ({
        id: k.id,
        display: k.hiragana,
        romaji: k.romaji,
...
export function makeWordToImageGenerator(
  ...
  return () => {
    ...
    idx++;
    const distractors = shuffled(
      validConcepts.filter((cw) => cw.id !== correct.id),
    ).slice(0, maxBubbles - 1);
    const items: BubbleItem[] = shuffled([correct, ...distractors]).map(
      (cw) => ({
        id: cw.id,
